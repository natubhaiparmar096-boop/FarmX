...
public class GameStateHandler {
    ...
    public void onTablistUpdate(UpdateTablistEvent event) {
        ...
        // FakePixel / non-Hypixel: no "Area: Garden" on tablist — treat as Garden so farming works
    ...
    public void checkBuffsTabList(List<String> footerString) {
        ...
        if (!loaded) {
            cookieBuffState = BuffState.ACTIVE; // FakePixel
            godPotState = BuffState.UNKNOWN;
            return;
        }

        cookieBuffState = BuffState.ACTIVE; // FakePixel: always treat cookie as active
    ...
    public void scheduleRewarp() {
        ...
        reWarpTimer.schedule(); // start cooldown so FakePixel skip-warp doesn't spam
    }
    ...
    public boolean inGarden() {
        // Always treat player as in garden (FakePixel / non-Hypixel servers don't report Garden area)
